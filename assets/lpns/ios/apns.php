    ...
    */  
        ...
        {  
            ...
            protected function getBinaryNotification($configToken, $payload, $messageID = 0, $Expire = 604800)  
            {  
                $tokenLength = strlen($configToken);  
                $payloadLength = strlen($payload);  
      
                $ret  = pack('CNNnH*', self::COMMAND_PUSH, $messageID, $Expire > 0 ? time() + $Expire : 0, self::config_BINARY_SIZE, $configToken);  
            ...
            {  
                ...
                foreach($this->_configTokens AS $key => $value)  
                {  
            ...
            public function addDT($configToken)  
            {  
                if (!preg_match('~^[a-f0-9]{64}$~i', $configToken))   
                {  
                    throw new Exception  
                    (  
                        "Invalid config token '{$configToken}'"  
                    );  
                }  
                $this->_configTokens[] = $configToken;  
            }         
              
            public function getDTNumber()  
            {  
                return count($this->_configTokens);  
            }  
    ...
    $rootpath = 'entrust_root_certification_authority.pem';  //ROOT֤���ַ  
    $cp = 'apns-dev.pem';  //provider֤���ַ  
    $apns = new APNS(1,$cp);  
    try  
    {  
        //$apns->setRCA($rootpath);  //���ROOT�֤�  
        $apns->connect(); //���  
        $apns->addDT('30ef38ec404d03f78fe9798134d3f4f27502d1fa6f38386b125919d81051f1af');  //Ӽ��configToken  
        $apns->setText('�����һ�������Ϣ');  //�������  
        $apns->setBadge(1);  //�����ͼ���  
        $apns->setSound();  //��������  
        $apns->setExpiry(3600);  //����ʱ�  
        $apns->setCP('custom operation',array('type' => '1','url' => 'http://www.google.com.hk'));  //��Զ�����  
        $apns->send();  //��  
        echo ' sent ok';  
    }catch(Exception $e)  
    {  
